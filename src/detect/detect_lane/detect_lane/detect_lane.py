...
class detect(Node):
    def __init__(self):
        ...
        # 顏色選擇器參數
        self.declare_parameter('color_picker_x', 320.0)  # 預設在圖像中央
        self.declare_parameter('color_picker_y', 240.0)
        ...
        # 獲取顏色選擇器參數
        self.color_picker_x = int(self.get_parameter('color_picker_x').value)
        self.color_picker_y = int(self.get_parameter('color_picker_y').value)

        MOV_AVG_LENGTH = 5
        self.lane_fit_bef = np.array([0, 0, 0])  # Initialize lane_fit_bef attribute   
        self.mov_avg_left = np.zeros((MOV_AVG_LENGTH, 3))
        self.mov_avg_right = np.zeros((MOV_AVG_LENGTH, 3))
        ...
        ## pub
        self.publisher_lane = self.create_publisher(Image, '/detect/lane', 1)
        ...
        if self.calibration_mode == True:
            # pub
            self.publisher_birdseye = self.create_publisher(Image, '/image/birdseye_image', 1)
            self.publisher_yellow = self.create_publisher(Image, '/image/yellow_image', 1)
            self.publisher_white = self.create_publisher(Image, '/image/white_image', 1)
            self.pub_calib = self.create_publisher(Image, '/image/image_calib',1)
            
            # 顏色選擇器發布器 - 修改為針對原始影像
            self.pub_hsv_value = self.create_publisher(Float64MultiArray, '/detect/hsv_value', 1)
            self.pub_original_with_picker = self.create_publisher(Image, '/image/original_with_picker', 1)
            ...
            # 顏色選擇器訂閱
            self.subscription_color_picker_x = self.create_subscription(
                Float64, '/detect/parameter/color_picker_x', self.color_picker_x_callback, 1)
            self.subscription_color_picker_y = self.create_subscription(
                Float64, '/detect/parameter/color_picker_y', self.color_picker_y_callback, 1)
    ...
    def color_picker_x_callback(self, msg):
        self.color_picker_x = int(msg.data)
        
    def color_picker_y_callback(self, msg):
        self.color_picker_y = int(msg.data)
    ...
    def image_callback(self, msg):
        ...
        top_x1, top_y1 = center_x - top_width // 2, top_height
        top_x2, top_y2 = center_x + top_width // 2, top_height
        bottom_x1, bottom_y1 = center_x - bottom_width // 2, bottom_height 
        bottom_x2, bottom_y2 = center_x + bottom_width // 2, bottom_height 
        ...
        if self.calibration_mode == True:
            # 在原始圖像上添加顏色選擇器的十字標記
            original_with_picker = cv_image_original.copy()
            
            # 確保座標點在有效範圍內
            x = min(max(0, self.color_picker_x), new_width - 1)
            y = min(max(0, self.color_picker_y), new_height - 1)
            
            # 繪製十字線
            cv2.line(original_with_picker, (x - 10, y), (x + 10, y), (0, 0, 255), 2)  # 水平線
            cv2.line(original_with_picker, (x, y - 10), (x, y + 10), (0, 0, 255), 2)  # 垂直線
            
            # 獲取該點的 HSV 值
            hsv_image = cv2.cvtColor(cv_image_original, cv2.COLOR_BGR2HSV)
            hsv_value = hsv_image[y, x]
            
            # 在圖像上顯示 HSV 值
            cv2.putText(original_with_picker, f"HSV: {hsv_value}", 
                       (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            cv2.imshow
            # 發布 HSV 值
            hsv_msg = Float64MultiArray()
            hsv_msg.data = [float(hsv_value[0]), float(hsv_value[1]), float(hsv_value[2])]
            self.pub_hsv_value.publish(hsv_msg)
            cv2.imshow('Detected', original_with_picker)
            cv2.waitKey(1)
            # 發布帶有顏色選擇器的原始圖像
            self.pub_original_with_picker.publish(self.cv_bridge.cv2_to_imgmsg(original_with_picker, encoding='bgr8'))
        ...
        if self.calibration_mode == False:
            ...
            rclpy.logging.get_logger('detect_node').info("mov_avg_left")
            rclpy.logging.get_logger('detect_node').info(str(type(self.mov_avg_left)))
    ...
    def make_dul_lane(self, cv_image, white_fraction, yellow_fraction):
        ...
        centerx = np.zeros(480, dtype=np.float64)
        ...
        if self.reliability_white_line > 50 and self.reliability_yellow_line > 50:   
            if white_fraction > 4000 and yellow_fraction > 4000: #3000
                ...
                # print('==================================')
                # print(type(centerx))
                # rclpy.logging.get_logger('detect_node').info(str(type(centerx)))
            ...
            if white_fraction > 4000 and yellow_fraction <= 4000: #3000
                print('hi2')
                centerx = np.subtract(self.right_fitx, value_r) #320
                pts_center = np.array([np.transpose(np.vstack([centerx, ploty]))])
            ...
            if white_fraction <= 4000 and yellow_fraction > 4000: #3000
                print('hi3')
                centerx = np.add(self.left_fitx, value) #320
                pts_center = np.array([np.transpose(np.vstack([centerx, ploty]))])
        ...
        #     msg_desired_center.data = centerx.item(300) #240
        #     if self.lane_toggle == True:
        #         self.publisher_control_lane.publish(msg_desired_center)


        if np.any(centerx >= 480):
            centerx[300] = 480
        elif np.any(centerx <= 0):
            centerx[300] = 0
        ...
        # publishes lane center
            ...
            self.msg_desired_center = msg_desired_center.data

        else:
            msg_desired_center = Float64()
            msg_desired_center.data = self.msg_desired_center
    ...
    def make_white_lane(self, cv_image, white_fraction):
        ...
        # # publishes lane center
        #     msg_desired_center = Float64()
        #     msg_desired_center.data = centerx.item(350) #230
        #     if self.lane_toggle == True:
        #         self.publisher_control_lane.publish(msg_desired_center)
