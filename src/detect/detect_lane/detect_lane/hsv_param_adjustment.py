...
from PyQt5.QtWidgets import QApplication, QMainWindow, QVBoxLayout, QHBoxLayout, QSlider, QLabel, QWidget, QPushButton, QMessageBox, QFileDialog
...
class ParameterAdjuster(Node):
    def __init__(self):
        ...
        # 添加顏色選擇器參數
        self.declare_parameter('color_picker_x', 320.0)
        ...
        # 添加顏色選擇器發布器
        self.publisher_color_picker_x = self.create_publisher(Float64, '/detect/parameter/color_picker_x', QoSProfile(depth=10))
        self.publisher_color_picker_y = self.create_publisher(Float64, '/detect/parameter/color_picker_y', QoSProfile(depth=10))
    ...
    def save_parameters_to_yaml(self):
        ...
        parameter_names = [
            ...
            'color_picker_x', 'color_picker_y'  # 添加顏色選擇器參數
        ]
...
class ColorPickerWindow(QMainWindow):
    def __init__(self, adjuster_node):
        ...
        self.setWindowTitle('Color Picker Adjustment')
        ...
        self.x_layout.addWidget(self.label_x)
        
        self.slider_x = QSlider()
        self.slider_x.setMinimum(0)
        self.slider_x.setMaximum(640)
        self.slider_x.setValue(320)  # 預設在圖像中央
        self.slider_x.setOrientation(1)  # 水平方向
        self.x_layout.addWidget(self.slider_x)
        
        self.value_x = QLabel('320')
        self.x_layout.addWidget(self.value_x)
        ...
        self.y_layout.addWidget(self.label_y)
        
        self.slider_y = QSlider()
        self.slider_y.setMinimum(0)
        self.slider_y.setMaximum(480)
        self.slider_y.setValue(240)  # 預設在圖像中央
        self.slider_y.setOrientation(1)  # 水平方向
        self.y_layout.addWidget(self.slider_y)
        
        self.value_y = QLabel('240')
        self.y_layout.addWidget(self.value_y)
        
        self.layout.addLayout(self.y_layout)
        
        # 連接滑塊變動事件
        self.slider_x.valueChanged.connect(self.slider_x_changed)
        self.slider_y.valueChanged.connect(self.slider_y_changed)
    
    def slider_x_changed(self, value):
        self.value_x.setText(str(value))
        self.adjuster_node.adjust_parameters('color_picker_x', float(value))
    
    def slider_y_changed(self, value):
        self.value_y.setText(str(value))
        self.adjuster_node.adjust_parameters('color_picker_y', float(value))
...
class SliderWindow(QMainWindow):
    def __init__(self, adjuster_node):
        ...
        # 顏色選擇器窗口實例
        self.color_picker_window = ColorPickerWindow(self.adjuster_node)
...
def main():
    ...
    # 在主窗口顯示的同時，顯示顏色選擇器窗口
    window.color_picker_window.show()
