...
class detect(Node):
    def __init__(self):
        ...
        self.declare_parameter('boundcenter_y', 107.0)
        self.declare_parameter('boundwidth', 60.0)
        self.declare_parameter('boundheight', 120.0)
        
        self.declare_parameter('hue_red_l', 0.0)
        self.declare_parameter('hue_red_h', 26.0)
        self.declare_parameter('saturation_red_l', 70.0)
        self.declare_parameter('saturation_red_h', 111.0)
        ...
        self.declare_parameter('hue_green_l', 74.0)
        self.declare_parameter('hue_green_h', 101.0)
        self.declare_parameter('saturation_green_l', 3.0)
        self.declare_parameter('saturation_green_h', 40.0)
        self.declare_parameter('lightness_green_l', 200.0)
        ...
        self.current_image_for_mouse = None  # 新增：儲存目前顯示的影像
        cv2.namedWindow('Detected Circles')
        cv2.setMouseCallback('Detected Circles', self.mouse_callback)
    ...
    def image_callback(self, msg):
        ...
        if self.hsv is not None:
            cv2.putText(cv_image_original, f'HSV: {self.hsv}', (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2)
        cv2.imshow('Detected Circles', cv_image_original)
        self.current_image_for_mouse = cv_image_original  # 新增：更新目前顯示的影像
        cv2.waitKey(1)
    ...
    def mouse_callback(self, event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN and self.current_image_for_mouse is not None:
            # 取得點擊位置的 BGR 值
            bgr = self.current_image_for_mouse[y, x]
            # 轉換為 HSV
