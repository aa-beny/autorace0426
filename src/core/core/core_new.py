...
class node(Node):
    ...
    def yellow_fraction_callback(self, msg):
        ...
        # self.get_logger().info('Received: yellow_fraction_callback sign')
...
#==================================mode select===============================================
        if self.mode.value == Mode.LANE.value:
            ...
            if self.yellow_fraction < 3000 and self.white_fraction < 3000 and self.TUNNEL_done ==False:
                self.stop_fun()
                pub_lane_msg = Bool()
                pub_lane_msg.data = False
                self.pub_lane_toggle.publish(pub_lane_msg)
            ...
            #     self.get_logger().error('into launch')
            #     self.navigation_launch_ls.run()
            #     self.get_logger().error('out launch')
            ...
            # # # elif self.TUNNEL_done ==True:    ## detect lane go on
            #     self.get_logger().info('in LANE')
            #     pub_lane_msg = Bool()
            #     pub_lane_msg.data = True
            #     self.pub_lane_toggle.publish(pub_lane_msg)
